...
| sed -E -e "s/\[.*:(.*)\].*startup and initiation for all shards.*took (.*)ms for (.*) shards.*/$i,\1,startupAndInitiate,\2,\3/" \
         -e "s/\[.*:(.*)\].*startup and initiation for the config server.*took (.*)ms with (.*) nodes.*/$i,\1,startupAndInitiateConfigServer,\2,\3/" \
